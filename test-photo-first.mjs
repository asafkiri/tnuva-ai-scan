...
import { tnuvaPaperCheck, scanChecksumMismatches, createServer } from './server.js';
...
async function request(responses, documents = input) {
  const calls = [], logs = [];
...
test('normal photo: one Luna request and complete audit', async () => {
  const { output, calls, logs } = await request([answer(doc())]);
...
test('wrong printed line count: one Terra priority retry, good result selected and both calls retained', async () => {
  const { output, calls } = await request([answer(doc({ itemsPrintedLines: 2 })), answer(doc())]);
  assert.equal(calls.length, 2);
  assert.equal(calls[1].model, 'gpt-5.6-terra');
  assert.equal(calls[1].service_tier, 'priority');
  assert.equal(output.model, 'gpt-5.6-terra');
  assert.equal(output.paperValidation[0].ok, true);
  assert.deepEqual(output.scanAudit.attempts.map(a => a.selected), [false, true]);
});
test('failed or worse retry preserves the first result and records the failure', async () => {
  const first = doc({ subtotalExVat: 49 });
  for (const second of [new Error('test network failure'), answer(doc({ subtotalExVat: null }))]) {
    const { output, calls } = await request([answer(first), second]);
    assert.equal(calls.length, 2);
...
test('missing summary asks for the missing page without an automatic paid reread', async () => {
  const { output, calls } = await request([answer(doc({ itemsPrintedLines: null }))]);
  assert.equal(calls.length, 1);
...
test('self-consistent paper disproves a wrong typed anchor without escalation', async () => {
  const { output, calls } = await request([answer(doc())], [{ ...input[0], expectedSubtotalExVat: 60 }]);
  assert.equal(calls.length, 1);
  assert.equal(output.error, 'anchor_mismatch_printed');
  assert.equal(output.scanAudit.attempts.length, 1);
});
test('invalid initial output is audited and never blindly retried', async () => {
  const { output, calls } = await request([{ output_text: 'invalid json' }]);
  assert.equal(calls.length, 1);
...
test('quantity typo triggers one corrective read even when document totals and line counts match', async () => {
  const { output, calls } = await request([answer(doc({ rows: [row({ quantity: 9 })] })), answer(doc())]);
  assert.equal(calls.length, 2);
  assert.equal(output.model, 'gpt-5.6-terra');
  assert.equal(output.paperValidation[0].ok, true);
});
...
test('valid mixed and credit photos return review guidance without another model call', async () => {
  for (const d of [doc({ docType: 'credit', subtotalExVat: -50 }), doc({ subtotalExVat: 40,
    ...
    const { output, calls } = await request([answer(d)]);
    assert.equal(calls.length, 1);
